...
[{/if}]
    ...
    <p>[{ oxmultilang ident="MO_OGONE__REFUND_EXPLANATION"}]</p>
    <table cellspacing="0" cellpadding="0" border="0" width="98%">
    <form name="search" id="search" action="[{ $oViewConf->getSelfLink() }]" method="post">
        [{ $oViewConf->getHiddenSid() }]
        <input type="hidden" name="cl" value="mo_ogone__refund">
        <input type="hidden" name="oxid" value="[{ $oxid }]">
    ...
    [{if $deliveryPrice->getBruttoPrice() != 0 }]
        <input type="checkbox" name="includeShipment" value="1" >[{ oxmultilang ident="MO_OGONE__REFUND_INCLUDE_SHIPMENT"}]</br>
    [{/if}]
    [{if $edit->getGiftCard() }]
        <input type="checkbox" name="includeGiftcard" value="1" >[{ oxmultilang ident="MO_OGONE__REFUND_INCLUDE_GIFTCARD"}]</br>
    [{/if}]

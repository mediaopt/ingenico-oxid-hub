...
<form name="transfer" id="transfer" action="[{$oViewConf->getSelfLink()}]" method="post">
    ...
    <input type="hidden" name="cl" value="[{$mo_ingenico__cl}]">
</form>
[{if !$edit->mo_ingenico__isIngenicoOrder()}]
    this is not an ingenico payment
[{else}]
<link rel="stylesheet" href="[{$oViewConf->getModuleUrl('mo_ingenico','out/src/css/admin/mo_ingenico__admin.css')}]">
[{if $success_message}]
...
[{/if}]
    ...
    <p>[{oxmultilang ident="MO_INGENICO__CAPTURE_EXPLANATION"}]</p>
    <table cellspacing="0" cellpadding="0" border="0" width="98%">
    <form name="search" id="search" action="[{$oViewConf->getSelfLink()}]" method="post">
        [{$oViewConf->getHiddenSid()}]
        <input type="hidden" name="cl" value="[{$mo_ingenico__cl}]">
        <input type="hidden" name="oxid" value="[{$oxid}]">
        <input type="hidden" name="fnc" value="[{$mo_ingenico__fnc}]">
    <tr>
    ...
    [{if $deliveryPrice->getBruttoPrice() != 0}]
        <input type="checkbox" name="includeShipment" value="1" >[{oxmultilang ident="MO_INGENICO__CAPTURE_INCLUDE_SHIPMENT"}]</br>
    [{/if}]
    [{if $edit->getGiftCard()}]
        <input type="checkbox" name="includeGiftcard" value="1" >[{oxmultilang ident="MO_INGENICO__CAPTURE_INCLUDE_GIFTCARD"}]</br>
    [{/if}]

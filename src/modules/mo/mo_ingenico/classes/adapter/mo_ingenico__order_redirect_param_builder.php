...
class mo_ingenico__order_redirect_param_builder extends mo_ingenico__request_param_builder
{
    ...
    public function build()
    {
        ...
        $redirectUrl = $this->checkUrlLength(
            ...
            'index.php?cl=order&fnc=mo_ingenico__fncHandleIngenicoRedirect', 200);
        ...
        if ($this->getOxConfig()->getConfigParam('ingenico_blBackButton') == 'true') {
            $params['backurl'] = $this->checkUrlLength($this->getOxConfig()->getSslShopUrl(), 200);
        }

        $params['pm'] = Main::getInstance()->getService('IngenicoPayments')->getPaymentMethodProperty($this->getOxSession()->getBasket()->getPaymentId(), 'pm');

        // if credit card is used and is using redirect (hidden auth is disabled), use the brand that was submitted
        if ($this->getOxSession()->getBasket()->getPaymentId() == 'ingenico_credit_card') {
            $dynvalues = $this->getOxSession()->getVariable('dynvalue');
            $params['brand'] = $dynvalues['mo_ingenico']['cc']['brand'];
        } else {
            $brand = Main::getInstance()->getService('IngenicoPayments')->getPaymentMethodProperty($this->getOxSession()->getBasket()->getPaymentId(), 'brand');
            $params['brand'] = is_array($brand) ? $brand[0] : $brand;
        }
        // shop logo
        if ($this->getOxConfig()->getConfigParam('ingenico_sTplLogo') !== '') {
            $params['logo'] = $this->getOxConfig()->getConfigParam('ingenico_sTplLogo');
        }

        // dynamic template
        if ($this->getOxConfig()->getConfigParam('ingenico_sTemplate') == 'true') {
            $params['tp'] = $this->getOxConfig()->getSslShopUrl() . 'index.php?cl=mo_ingenico__template&' . http_build_query($this->getOxidSessionParamsForRemoteCalls());

            // static template
        } else {
            $params['title'] = $this->getStaticTitle();
            $params['bgcolor'] = $this->getOxConfig()->getConfigParam('ingenico_sTplBGColor');
            $params['txtcolor'] = $this->getOxConfig()->getConfigParam('ingenico_sTplFontColor');
            $params['tblbgcolor'] = $this->getOxConfig()->getConfigParam('ingenico_sTplTableBGColor');
            $params['tbltxtcolor'] = $this->getOxConfig()->getConfigParam('ingenico_sTplTbFontColor');
            $params['buttonbgcolor'] = $this->getOxConfig()->getConfigParam('ingenico_sTplBtnBGColor');
            $params['buttontxtcolor'] = $this->getOxConfig()->getConfigParam('ingenico_sTplBtnFontColor');
            $params['fonttype'] = $this->getOxConfig()->getConfigParam('ingenico_sTplFontFamily');
        }

        // billpay extra params
        if ($this->getOxSession()->getBasket()->getPaymentId() == 'ingenico_open_invoice_de') {
            $params = $this->addBillPayParams($params);
        }

        // paypal extra params
        if ($this->getOxSession()->getBasket()->getPaymentId() == 'ingenico_paypal') {
    ...
    protected function addBillPayParams($params)
    {
        ...
        $date = explode('-', $dynvalues['mo_ingenico']['invoice']['birthdate']);
    ...
    {
        if (!$this->getOxConfig()->getConfigParam('ingenico_blTplTitle')) {
            return '';
        }

        $oxContent = oxNew('oxContent');
        if ($oxContent->loadByIdent('mo_ingenico_tplTitle') &&
